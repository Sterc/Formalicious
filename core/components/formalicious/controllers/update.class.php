...
class FormaliciousUpdateManagerController extends FormaliciousBaseManagerController
{
    ...
    public function process(array $scriptProperties = [])
    {
        if (isset($scriptProperties['id'])) {
            $this->form = $this->modx->getObject('FormaliciousForm', [
                'id' => $scriptProperties['id']
            ]);
        } else {
            $this->form = $this->modx->newObject('FormaliciousForm');
        }

        if ($this->form === null) {
            $this->failure($this->modx->lexicon('formalicious.form_not_exists', [
                'id' => $scriptProperties['id']
            ]));
        }
    ...
    public function getFieldTypes()
    {
        ...
        $query = $this->modx->newQuery('FormaliciousFieldType');
        $query->sortby('name', 'ASC');

        foreach ($this->modx->getCollection('FormaliciousFieldType', $query) as $fieldType) {
