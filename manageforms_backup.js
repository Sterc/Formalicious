...
Ext.extend(Formalicious.grid.Values,MODx.grid.LocalGrid,{
    ...
    ,updateRole: function(btn,e) {
        ...
        console.log('ipdate')
        this.windows.updateValue = MODx.load({
            xtype: 'formalicious-window-update-value'
            ,force:true
            ,record: r
            ,listeners: {
                'success': {fn:function(r) {
        ...
        this.windows.updateValue.fp.getForm().reset();
        this.windows.updateValue.fp.getForm().setValues(r);
        this.windows.updateValue.show(e.target);
    }
    ,getMenu: function() {
        return [{
...
Formalicious.window.AddValue = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,id: 'formalicious-window-add-value'
        ,height: 250
        ,width: 450
        ,modal: true
        ,saveBtnText: _('done')
...
Formalicious.window.UpdateValue = function(config) {
    config = config || {};
    Ext.applyIf(config,{
        title: _('inyourface.field.create')
        ,id: 'formalicious-window-update-value'
        ,height: 250
        ,width: 450
        ,modal: true
        ,saveBtnText: _('done')
    ...
    console.log(config);
    Formalicious.window.UpdateValue.superclass.constructor.call(this,config);
};
Ext.extend(Formalicious.window.UpdateValue,MODx.Window,{
    submit: function() {
        if (this.fp.getForm().isValid()) {
            if (this.fireEvent('success',this.fp.getForm().getValues())) {
                this.fp.getForm().reset();
...
Ext.reg('formalicious-window-update-value',Formalicious.window.UpdateValue);
